...
def call_claude_for_formatting(extracted_data):
    ...
    # Prepare logo tag for header - larger with address below
    logo_html = ""
    if HAS_LOGO:
        logo_html = f'<div><img src="file://{LOGO_PATH}" style="height: 90px; width: auto; display: block; margin-bottom: 5pt;"><div style="font-size: 9pt; color: #555;">Unit 1001, 115 Schoolhouse St, Coquitlam, BC, V3K 4X8</div></div>'
        print(f"✅ Logo will be included from: {LOGO_PATH}")
    else:
        logo_html = '<div><div style="font-size: 22pt; font-weight: bold; color: #2B4C7E;">ALL PAINTING LTD.</div><div style="font-size: 10pt; color: #555;">Your Full-Service Painting Company</div><div style="font-size: 9pt; color: #555; margin-top: 5pt;">Unit 1001, 115 Schoolhouse St, Coquitlam, BC, V3K 4X8</div></div>'
...
```html
<div style="margin-bottom: 12pt; page-break-inside: avoid !important;">
    <div style="display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 12pt; border-bottom: 3px solid #2B4C7E;">
        {logo_html}
    ...
    <div style="text-align: center; font-size: 10pt; color: #555; padding-top: 8pt;">
        Phone: (604) 525-1403 | Email: info@allpaintingltd.ca | Website: www.allpaintingltd.ca
    </div>
</div>
```
