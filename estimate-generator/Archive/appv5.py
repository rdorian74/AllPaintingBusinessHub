...
def call_claude_for_formatting(extracted_data):
    ...
    # Prepare logo tag for header - larger since no text next to it
    logo_html = ""
    if HAS_LOGO:
        logo_html = f'<div style="background: white; border-radius: 8px; padding: 8pt;"><img src="file://{LOGO_PATH}" style="height: 70px; width: auto; display: block;"></div>'
        print(f"✅ Logo will be included from: {LOGO_PATH}")
    else:
        logo_html = '<div style="font-size: 20pt; font-weight: bold;">ALL PAINTING LTD.</div><div style="font-size: 10pt;">Your Full-Service Painting Company</div>'
...
3. EXECUTIVE SUMMARY (NEVER BREAK - class="section no-break", page-break-inside: avoid !important; margin-bottom: 12pt):
   ...
   - Data rows (11pt)
   - Right-align amounts
   - SUBTOTAL row at bottom (navy #2B4C7E background, white 12pt bold text): ${subtotal:,.2f}
...
5. STANDARD INCLUSIONS (ALLOW BREAKS IF LONG - class="section", margin-bottom: 12pt):
   ...
   ```
   NOTE: Use standard bullet points (list-style-type: disc), NOT checkmarks. Single column layout, proper indentation.
...
7. EXCLUSIONS (ALLOW BREAKS IF LONG - class="section", margin-bottom: 12pt):
   ...
   IMPORTANT: GST (Goods and Services Tax @ 5%) MUST ALWAYS be the FIRST item in exclusions list.
...
14. GST HANDLING - GST is EXCLUDED. Show only SUBTOTAL in Executive Summary (no GST row, no Total row). GST must be FIRST item in Exclusions.
15. STANDARD INCLUSIONS - Use standard bullet points (disc), single column, NO checkmarks.
