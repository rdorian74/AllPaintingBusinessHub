...
11. ADDENDUM EXTRACTION: Look for text like "Addendum 1", "Addendum 2", "Addendum #3", etc. Extract ONLY the number (e.g., "3" not "Addendum #3"). If NO addendum is found, return "No addendums"
...
18. PREPARED BY: Look for who prepared/signed the estimate. Look for patterns like "Prepared by:", "Estimator:", "By:", signature lines, or names at the end of the document (e.g., "Ian Cooke", "John Smith"). Extract the person's name who prepared/signed the document.
...
def call_claude_for_formatting(extracted_data):
    ...
    logo_html = ""
    if HAS_LOGO:
        logo_html = f'<div><img src="file://{LOGO_PATH}" style="height: 90px; width: auto; display: block; margin-bottom: 5pt;"><div style="font-size: 9pt; color: #555; line-height: 1.4;">Unit 1001, 115 Schoolhouse St, Coquitlam, BC, V3K 4X8<br>Phone: (604) 525-1403 | Email: info@allpaintingltd.com<br>www.allpaintingltd.com</div></div>'
        print(f"✅ Logo will be included from: {LOGO_PATH}")
    else:
        logo_html = '<div><div style="font-size: 22pt; font-weight: bold; color: #2B4C7E;">ALL PAINTING LTD.</div><div style="font-size: 10pt; color: #555;">Your Full-Service Painting Company</div><div style="font-size: 9pt; color: #555; margin-top: 5pt; line-height: 1.4;">Unit 1001, 115 Schoolhouse St, Coquitlam, BC, V3K 4X8<br>Phone: (604) 525-1403 | Email: info@allpaintingltd.com<br>www.allpaintingltd.com</div></div>'
...
<div style="margin-bottom: 12pt; page-break-inside: avoid !important; border-bottom: 3px solid #2B4C7E; padding-bottom: 12pt;">
    <div style="display: flex; justify-content: space-between; align-items: flex-start;">
        ...
        <div style="text-align: right;">
            ...
            <div style="font-size: 11pt; color: #333;">Addendum: [SEE ADDENDUM FORMATTING RULE BELOW]</div>
            <div style="font-size: 11pt; color: #333;">Valid for: {extracted_data.get('validity_days', '30')} days</div>
            <div style="font-size: 11pt; color: #333;">Prepared by: [prepared_by from data if available]</div>
        </div>
    </div>
</div>
```
...
- If addendum_count contains a number like "Addendum #3" or "Addendum 3" or just "3", extract the number and display as "Up to #[number]" (e.g., "Up to #3")
- If addendum_count is "No addendums" or empty or not found, display as "No addendums"
- NEVER display "Addendum: Addendum #3" - always convert to "Up to #3"

PREPARED BY RULE:
- Look for who signed/prepared the document (e.g., "Ian Cooke", "Prepared by:", "Estimator:", signature names)
- If found, display as "Prepared by: [name]"
- If not found, omit this line entirely
...
def upload_file():
    ...
    try:
        # Save original filename without extension for PDF naming
        original_name = os.path.splitext(file.filename)[0]
        filename = f"upload_{datetime.now().strftime('%Y%m%d_%H%M%S')}{os.path.splitext(file.filename)[1]}"
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(filepath)
        
        # Save original name to a companion file for later use
        original_name_file = os.path.join(app.config['UPLOAD_FOLDER'], f"{os.path.splitext(filename)[0]}_original.txt")
...
def process_document(filename):
    try:
        ...
        # Use "Estimate - " followed by the original Word document name
        if original_name:
            # Sanitize filename - remove characters that cause URL/filesystem issues
            ...
            output_filename = f"Estimate - {safe_name}.pdf"
        else:
...
def remove_working_from_name(filename):
    """
    Remove '(working)' or variations from filename.
    Handles: (working), (Working), (WORKING), ( working ), etc.
    """
    import re
    # Remove (working) with any spacing and case variations
    cleaned = re.sub(r'\s*\(\s*[Ww][Oo][Rr][Kk][Ii][Nn][Gg]\s*\)\s*', ' ', filename)
...
def api_generate():
    """
    Automated endpoint for estimate generation.
    
    Receives a Word document path (with or without "(working)" in name),
    renames the file to remove "(working)", processes it, and saves the PDF
    in the same folder as the Word document.
    
    Request JSON:
    {
        "word_file_path": "N:\\Estimates\\2026 All Painting Estimates\\25C-318 Project Name (working).docx"
    }
    
    Response JSON:
    {
        "success": true,
        "pdf_path": "N:\\Estimates\\2026 All Painting Estimates\\Estimate - 25C-318 Project Name.pdf",
        "pdf_filename": "Estimate - 25C-318 Project Name.pdf",
        "word_file_original": "...(working).docx",
        "word_file_renamed": "....docx",
        "extracted_data": { ... },
    ...
    try:
        ...
        if not data or 'word_file_path' not in data:
            return jsonify({
                'success': False,
                'error': 'Missing required field: word_file_path'
            }), 400
        ...
        if not os.path.exists(word_file_path):
            return jsonify({
                'success': False,
                'error': f'File not found: {word_file_path}'
            }), 404
        ...
        print("\n" + "="*60)
        print("API AUTOMATION - STARTING DOCUMENT PROCESSING")
        print("="*60)
        print(f"📁 Source file: {word_file_path}")
        
        # Get the directory and filename
        word_dir = os.path.dirname(word_file_path)
        word_filename = os.path.basename(word_file_path)
        word_extension = os.path.splitext(word_filename)[1]
        original_name = os.path.splitext(word_filename)[0]
        
        # Step 1: Rename Word file to remove "(working)"
        clean_name = remove_working_from_name(original_name)
        word_file_renamed = None
        
        if clean_name != original_name:
            new_word_filename = clean_name + word_extension
            new_word_path = os.path.join(word_dir, new_word_filename)
            
            print(f"\n📝 Step 1: Renaming Word file...")
            print(f"   From: {word_filename}")
            print(f"   To:   {new_word_filename}")
            
            # Check if destination already exists
            if os.path.exists(new_word_path):
                print(f"   ⚠️  Destination file already exists, will overwrite")
                os.remove(new_word_path)
            
            os.rename(word_file_path, new_word_path)
            word_file_path = new_word_path
            word_filename = new_word_filename
            word_file_renamed = new_word_path
            print(f"   ✅ File renamed successfully")
        else:
            print(f"\n📝 Step 1: No '(working)' found in filename, skipping rename")
        
        # Step 2: Extract text from document
        print(f"\n📄 Step 2: Extracting text from {word_filename}...")
        document_text = extract_text_from_doc(word_file_path)
        
        if not document_text or len(document_text) < 100:
            return jsonify({
                'success': False,
                'error': 'Could not extract sufficient text from document. Please ensure it is a valid Word document.'
            }), 500
        
        print(f"✅ Extracted {len(document_text)} characters")
        
        # Step 3: Use Claude to extract data
        print(f"\n🤖 Step 3: Using Claude AI to extract and polish data...")
        extracted_data = call_claude_for_extraction(document_text)
        
        # Step 4: Generate HTML
        print(f"\n🎨 Step 4: Generating professionally formatted HTML...")
        html_content = call_claude_for_formatting(extracted_data)
        
        # Step 5: Generate PDF filename and path (using clean name)
        # Sanitize filename - remove characters that cause issues
        ...
        pdf_filename = f"Estimate - {safe_name}.pdf"
        pdf_path = os.path.join(word_dir, pdf_filename)
        
        # Step 6: Generate PDF and save to same folder as Word document
        ...
        print("="*60)
        print("API AUTOMATION - PROCESSING COMPLETE!")
        print("="*60 + "\n")
        ...
        # Include rename info if file was renamed
        if word_file_renamed:
            response_data['word_file_original'] = data['word_file_path']
            response_data['word_file_renamed'] = word_file_renamed
